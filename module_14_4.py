...
async def get_buying_list(call: types.CallbackQuery):
    products = crud_functions.get_all_products()  # Получаем продукты из базы данных

    # Выводим информацию о каждом продукте
    for product in products:
        await call.message.answer(f'Название: {product[1]} | Описание: {product[2]} | Цена: {product[3]} руб.')
        with open(f'files/product{product[0]}.jpg', 'rb') as img:  # Загружаем картинки продуктов
